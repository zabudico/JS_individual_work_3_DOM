...
Событие при отправке формы:
   ...
   - Событие на форме (#transactionForm) вызывает функцию addTransaction(event), которая добавляет новую транзакцию при отправке формы.

### Инициализация на загрузке страницы:

   - После загрузки страницы вызываются функции renderTransactions() и calculateTotal() для отображения существующих транзакций и вычисления общей суммы.
